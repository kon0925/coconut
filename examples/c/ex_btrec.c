...
main __P((int, int **));
int	ex_btrec __P((int, int,int,int, int, int, char *, char *, int));
...
main(int argc, int **argv)
{
	...
	printf("\nCOCONUT+>>Indexing %s\n", dataset);
	return (ex_btrec(size, memory, exact, noise, indexing, leafsize, dataset, queries, lenz) == 1 ? EXIT_FAILURE : EXIT_SUCCESS);
}

int
ex_btrec(int ts_num, int memory, int EXACT, int noise, int indexing, int leafsize, char * dataset, char * queries, int lenz)
{
