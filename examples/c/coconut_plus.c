...
main(int argc, int **argv)
{
	...
	printf("\nCOCONUT+>>Indexing %s\n", dataset);
	return (coconut_plus(size, memory, exact, queriesnumber, indexing, leafsize, dataset, queries, lenz) == 1 ? EXIT_FAILURE : EXIT_SUCCESS);
}

int
coconut_plus(int ts_num, int memory, int EXACT, int queriesnumber, int indexing, int leafsize, char * dataset, char * queries, int lenz)
{
