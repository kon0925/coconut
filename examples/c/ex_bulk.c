...
#include <pthread.h>
#include <stdlib_mt.h>
#include "../../../iisax/src.haris/include/ads/sax/sax_breakpoints.h"
...
main __P((int, int **));
int	ex_btrec __P((int, int,int,int, int, int, char *, char *));
...
main(int argc, int **argv)
{
	...
	int noise=atoi(argv[4]);
	int indexing=atoi(argv[5]);
	int leafsize=atoi(argv[6]);
	char *dataset=argv[7];
	char *queries=argv[8];
	printf("\nCOCCONUT>>Indexing %s\n", dataset);
	return (ex_btrec(size, memory, exact, noise, indexing, leafsize, dataset, queries) == 1 ? EXIT_FAILURE : EXIT_SUCCESS);
}

int
ex_btrec(int ts_num, int memory, int EXACT, int noise, int indexing, int leafsize, char * dataset, char * queries)
{
	//SEARCH 10 PLACES BEFORE and 10 AFTER
	int range=leafsize;//500; // 1 leaf (100values), 10 leafs=(1000values), 100 leafs(10000 values)
	int debug=0;
	int overridesorting=1;
	int queriesno=100;

	
	// COCONUT FULL
	// Memory fits all: sort in memory (ts also) and then write
	// Memory does not fit all: a) either use sorted input (overridesorting=1) or b) sort the invSax and re-read sorted in order to write them

	if(overridesorting==1 && memory>=ts_num)
	{
		printf("\n Since everything fits in memory overridesorting should be 0");
		overridesorting=0;
	}
	else
	{
		printf("\n Input file should be already sorted");
	}
	...
	int timeseries_size=256;
        int paa_segments=16;
        int ts_values_per_paa_segment=16;
	...
	if(indexing==1)
	{
		...
		int recordsfit=(UINT32_MAX/2)/(sizeof(float)*256);
//////////////////////////////// NEW CODE
	time_t current_time;
	double duration;
	double ioduration=0;
		...
		while(cnt<ts_num)
		{
			...
			if(ts_num>memory)
			{
				...
				while(tmpcnt<memory && cnt<ts_num)
				{
					...
					if(overridesorting==1)
					{
						sax_from_ts2( ((float *)tsqueue[tmpcnt].ts), ((unsigned char *)  sax), ts_values_per_paa_segment,
						...
						//printf("\nbaaaaaaaaaaaaaaaaaaaaaaaaaaa %d %d", tmpcnt, cnt);fflush(stdout);
					}
